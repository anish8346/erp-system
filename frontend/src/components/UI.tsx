...
export const Button = ({ children, variant = "primary", size = "md", className = "", ...props }: ButtonProps) => {
  ...
  return (
    <button 
      className={`rounded-lg font-semibold transition-all duration-200 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed ${variants[variant]} ${sizes[size]} ${className}`}
      {...props}
    >
      {children}
    </button>
...
export const ConfirmDialog = ({ 
  ...
  const variantStyles = {
    primary: "bg-luxury-brown hover:bg-[#3a2c20] text-white",
    danger: "bg-rose-600 hover:bg-rose-700 text-white",
    warning: "bg-amber-500 hover:bg-amber-600 text-white"
  };

  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-luxury-brown/20 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 border border-soft-cream">
        <div className="p-6">
          <h3 className={`text-lg font-bold ${variant === 'danger' ? 'text-rose-600' : variant === 'warning' ? 'text-amber-600' : 'text-luxury-brown'}`}>{title}</h3>
          <p className="text-sm text-warm-taupe font-medium mt-2 leading-relaxed">{description}</p>
        </div>
        <div className="px-6 py-4 bg-faded-white border-t border-soft-cream flex justify-end gap-3">
          {!isAlert && <Button variant="secondary" onClick={onClose} size="sm">{cancelText}</Button>}
          <Button 
